...
import com.rio.weatherapp.domain.use_cases.SearchCityUseCase
import com.rio.weatherapp.presentation.search.SearchStore.Intent
import com.rio.weatherapp.presentation.search.SearchStore.Label
import com.rio.weatherapp.presentation.search.SearchStore.State
    ...
    private inner class ExecutorImpl(private val openReason: OpenReason) :
        ...
        override fun executeIntent(intent: Intent, getState: () -> State) {
            when (intent) {
                ...
                is Intent.ClickCity -> {
                    when (openReason) {
                        OpenReason.AddCityToFavourite -> {
                            scope.launch {
                                changeCityFavouriteUseCase.addToFavourite(intent.city)
                                publish(Label.SavedToFavourite)
                            }
                        }

                        OpenReason.SearchCityForecast -> {
